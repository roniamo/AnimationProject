...
void BasicScene::KeyCallback(Viewport* viewport, int x, int y, int key, int scancode, int action, int mods)
{
    ...
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        case GLFW_KEY_T: {
            ...
            break;
        case GLFW_KEY_D: {
            Eigen::Matrix4f cubeTransform = autoCube->GetAggregatedTransform();
            Eigen::Vector3f cubeLoction = Eigen::Vector3f(cubeTransform.col(3).x(), cubeTransform.col(3).y(), cubeTransform.col(3).z());
            std::cout << "target = (x: " << cubeLoction.x() << " y: " << cubeLoction.y() << " z: " << cubeLoction.z() << ")" << std::endl; }
            break;
        case GLFW_KEY_N: {
